...
public class QueueWithPriority<T> implements IQueueWithPriority<T>{
    ...
    public void add(T a, int priority) {
        if (this.first == null) { // 0 elements
            this.first = new PriorityNode<T>(a, priority, null);
            return;
        }

        if (this.first.getNext() == null) { // 1 element
            if (this.first.getPriority() > priority) {
                this.first = new PriorityNode<T>(a, priority, this.first);
            } else {
                this.first.setNext(new PriorityNode<T>(a, priority, null));
            }
            return;
        }
        ...
        PriorityNode<T> backup = this.first;
        PriorityNode<T> candidate = this.first.getNext();
        while (candidate.getNext() != null) {
            if (candidate.getPriority() > priority) {
                backup.setNext(new PriorityNode<T>(a, priority, candidate));
                return;
            }
            candidate = candidate.getNext();
        }
        if (candidate.getPriority() > priority) {
            backup.setNext(new PriorityNode<T>(a, priority, null));
        }
    }
